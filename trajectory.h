...
// Modified by: Luca Frix
// Date 2023-12-8
// Making it work with microseconds
...
public:
	// Constructors
	Trajectory(float _thresh = 0.1, float _maxVel = 100, float _setAcc = 50, float _setDec = -1);
	...
	// dT = time since update or reset function was last called in microseconds
 ...
 *
 * @param  (_targetPos) New target position value
 * @param  (_time) The time in miliseconds until system should reach target position
